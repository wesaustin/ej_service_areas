...
cbg_HB_vio <- readRDS("Data/combined/area/HB_vio_epic_area.rds") %>%
...
#    0.00    0.00    0.00    0.99    1.00  366.00   86682
...
# 0.0000   0.0000   0.0000   0.9873   1.0000 366.0000 

state_sum_epic <- HB_vio_all %>%
  st_drop_geometry %>%
  group_by(state_name) %>%
  summarize(count = n(),
...
breaks <- list(bi_x = c("0", "1", "2", "300"),
               bi_y = c("0", "0.11", "0.64", "1"))
...
png(file = paste0(plot_path,"hb_POC_biv_US.png"), 
...
cbg_lcr_vio <- read_rds("Data/combined/area/lcr_vio_epic_area.rds")  %>%
  mutate(ID=str_pad(ID, 12, pad="0"))%>%
...
png(file = paste0(plot_path,"pb_lvl_lowinc_biv_US.png"), 
    width = 1915, height = 1077, units = "px", pointsize = 12,
    bg = "transparent")
...
png(file = paste0(plot_path,"pb_vio_POC_biv_US.png"), 
    width = 1915, height = 1077, units = "px", pointsize = 12,
    bg = "transparent")
...
png(file = paste0(plot_path,"pb_vio_lowinc_biv_US.png"), 
    width = 1915, height = 1077, units = "px", pointsize = 12,
    bg = "transparent")
