...
for(i in 1:length(zip_links)){
  ...
  data.zip <- read.table(con, header=T, skip=0, fill=T) %>% 
    write_csv(paste0("Data/zipcode_areas/",names(zip_links)[i],".csv"))
  head(data.zip)
...
zipcodeboundaries <- st_read("Data/zipcode_areas/tl_2019_us_zcta510/tl_2019_us_zcta510.shp") %>% 
  st_transform(4326) %>% 
...
st_write(zipcodeboundaries, "Data/zipcode_areas/generated_boundaries_zip/generated_boundaries_using_zip_codes.shp")
